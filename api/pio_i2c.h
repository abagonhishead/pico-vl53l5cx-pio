...
//Write function with no START signal at the end (used to send device address and register address before firmware write)
int pio_i2c_write_blocking_nostop(PIO pio, uint sm, uint8_t addr, uint8_t *txbuf, uint len);
//Write function with no START signal at the beginning (used to send firmware right after device address and register have been sent)
